...
final class SchemaCompiler
{
    ...
    private function compileCreateTable(Table $table, string $driver): string
    {
        ...
        return sprintf('CREATE TABLE %s (%s)', $this->quote($table->name, $driver), implode(', ', $columns));
    }
    ...
    private function columnSql(Column $column, string $driver): string
    {
        if ($column->autoIncrement && $column->primary) {
            return match ($driver) {
                ...
                'sqlsrv' => sprintf('%s BIGINT IDENTITY(1,1) PRIMARY KEY', $this->quote($column->name, $driver)),
                'oci' => sprintf('%s NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY', $this->quote($column->name, $driver)),
    ...
    private function mapType(string $type, string $driver): string
    {
        ...
        return match ($normalized) {
            'integer', 'int' => $driver === 'oci' ? 'NUMBER(10)' : 'INTEGER',
            'bigint' => $driver === 'oci' ? 'NUMBER(19)' : 'BIGINT',
            'string', 'varchar' => $driver === 'oci' ? 'VARCHAR2(255)' : ($driver === 'sqlsrv' ? 'NVARCHAR(255)' : 'VARCHAR(255)'),
            'text' => $driver === 'oci' ? 'CLOB' : ($driver === 'sqlsrv' ? 'NVARCHAR(MAX)' : 'TEXT'),
            ...
            'date' => 'DATE',
            'datetime', 'timestamp' => $driver === 'sqlsrv' ? 'DATETIME2' : 'TIMESTAMP',
            'json' => match ($driver) {
                'sqlite' => 'TEXT',
                'sqlsrv' => 'NVARCHAR(MAX)',
                'oci' => 'CLOB',
    ...
    {
        return $operation->tableDefinition ?? throw new \LogicException('Schema operation requires a table definition.');
    }

    private function requireColumn(SchemaOperation $operation): Column
    {
        return $operation->column ?? throw new \LogicException('Schema operation requires a column definition.');
    }
}
